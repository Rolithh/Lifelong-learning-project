...
#    train_data_loader = ContinualJacquardLoader(data_path)
    ...
    for task_i, task_data in enumerate(train_data_loader):
        ...
        #Training
        print('\n ----------  training task {}/{}  ---------- \n'.format(task_i, len(train_data_loader)-1))
        
        for epoch in range(n_epochs):
                    
            print('\n =============== (task ', task_i, ')   epoch {}  out of {}  =============== \n'.format(epoch, n_epochs))
            ...
            for i, batch in enumerate(task_data):
                ...
                norm_vect = torch.tensor([1024,  1024,  180,  1024, 1024]).float()
...
#                print('\n --Yn pred--- ', yn_pred[0] , '--Yn pred-- \n')
                ...
                old_loss = minMSELoss(yo_pred_normalized, yo_target_normalized)
                    ...
                    # Compute loss for current task (after normalizing gt)
                ...
                print("loss on each image of the batch : ", loss)
            ...
            #store loss value for every epoch in case we want to plot it later on 
            print('\n storing loss for plot... \n')
                        
            val = round(loss.mean().item(), 4)
            #fill the dictionary loss_by_task
            if task_i in loss_by_task:
                    loss_by_task[task_i].append(val)
            else:
                loss_by_task.setdefault(task_i, [val])
            
            
        print('\n saving model... \n')
        #save model
        torch.save(new_model.state_dict(), "model{}".format(task_i))
...
#print('\n --old loss--- ', old_loss , ' --old loss--- \n') 
#print('\n --new loss--- ', new_loss, ' --new loss--- \n')
